# deepspech training with cloud GPU
import os
import re
import string
import csv
...
def main():
    ...
    meta_path = '/content/meta'  # google collab path
    # meta_path = './meta/'  # local computer
    in_file = open(os.path.join(meta_path, 'txt.done.data'), 'r')  # using filename path
    # in_file = open(os.path.join(args.meta, 'txt.done.data'), 'r')  # commandline path
    ...
    try:
        for line in in_file:
            ...
            try:
                ...
                # fdur, fsr = get_audio_info(str(os.path.join(args.wav, fname)))  # commandline path
                # wav_path = './wav/'  # local computer
                wav_path = '/content/wav'
                fdur, fsr = get_audio_info(str(os.path.join(wav_path, fname)))  # using filename path
                #print("passes2")

                # Don't add files which don't fit into model specifications
                # Either not 48kHz or longer than 10 secs
                if fsr != 48000:
                    print("Different Sampling Rate:", fname)
                    continue
                if fdur > 10:
                ...
                fsize = os.path.getsize(os.path.join(wav_path, fname))
                print(fname, fsize, ftext)
                csv_writer.writerow([os.path.join(wav_path, fname), fsize, ftext])
                row_count += 1
